...
### Workspaces（云端工作区，#111）

云端工作区 = vault 根下一级目录 = 桌面端文件夹同步的绑定对象。一份注册表
（`STATE_DIR/workspaces.json`）同时服务 web / 桌面远程模式的工作区切换器和
桌面端同步绑定。继承自 sync-spaces 注册表（#109），迁移时保留原 id，
桌面端 `.quill/sync.json` 绑定不失效。

```
GET    /api/workspaces        → [{ id, name, remotePath, default?, createdAt, fileCount }]
POST   /api/workspaces        { name, remotePath }；重复 → 409，越权路径 → 400；同时创建目录
DELETE /api/workspaces/<id>   只删注册表记录，不动任何文件；默认工作区 → 400
```

启动时（幂等）：迁移 legacy sync-spaces.json → workspaces.json（保留 id）；
确保默认工作区 `quill`（`default: true`，不可删）+ 目录；vault 根下不属于任何
工作区的散文件移入 `quill/` 并打日志。

agent run 消息带 `workspaceId`，server 校验后把 `scope.root` 钉死到
`vault/<工作区目录>`；缺省 → 默认工作区；非法 id → 拒绝。

配套约定：vault 扫描（/index、/list）跳过 `.quill/` —— 桌面端在工作区文件夹内的
`.quill/sync.json` 里存绑定关系 + 上次同步的 hash 索引，同步元数据本身不上云。
